...
#pragma omp for schedule(dynamic) nowait
      for (ITYPE tilej = tileii; tilej < profileLength; tilej += maxTileWidth)
      {
        ...
        for (ITYPE jj = j; jj < MIN(tilej + maxTileWidth, profileLength); jj++)
        {
          ...
          {
            priv_profile_i[i - tilei] = correlation; // Updating private array
        ...
        if (tilei != tilej)
        {
          ...
          for (ITYPE ii = i; ii < MIN(MIN(tilei + maxTileHeight, j - exclusionZone), profileLength); ii++)
          {
            ...
            {
              priv_profile_i[ii - tilei] = correlation; // Updating private array
        ...
        // Transfer private profile data to global profile, then reset to -inf for the next tile
      ...
      } // Implicit OpenMP barrier if 'nowait' is not specified
#ifdef DEBUG
      cout << "-------------------------" << endl;
#endif
    }
...
int main(int argc, char *argv[])
{
  try
  {
    ...
    // The sizes of private tile arrays (maxTileWidth * 4 arrays * sizeof(DTYPE) * numThreads) are added to the size of profile and profileIndex
