...
{
  // With transactional memory, we will try to not privatize and access profile and indexProfile protecting them with a transaction
#pragma omp parallel
  {
    TX_DESCRIPTOR_INIT();
...
// NOT IMPORTANT
    ...
    {
      // Without protection when accessing profile it requires a barrier
    //BEGIN_ESCAPE;
#pragma omp for schedule(dynamic) nowait
      for (ITYPE tilej = tileii; tilej < profileLength; tilej += maxTileWidth)
      {
              //END_ESCAPE;
        // To iterate diagonally over tiles
...
// NOT IMPORTANT
        ...
        {
          // If i==j ==> Coordinates of the main diagonal. Only upper triangle is calculated.
          // Else, upper triangle is also calculated
          // Upper triangle
          covariance = 0;
        ...
        if (tilei != tilej)
        {
          // If the tile differs in its coordinates it is an inner tile and lower triangle will also be calculated
          // Lower triangle
          ITYPE i = tilei + 1;
          ITYPE j = tilej;
      ...
      }SB_BARRIER(tid); // Implicit omp barrier if nowait is not put
#ifdef DEBUG
...
int main(int argc, char *argv[])
{
  try
  {
    ...
    if(!statsFileInit(argc,argv,numThreads,MAX_XACT_IDS)){
      cout << "Error opening or initializing the stats file." << endl;
      return 0;
    }
    ...
    // The size doesn't change with the number of threads
    ...
    if(!dumpStats(telapsed.count())){
      cout << "Error dumping the statistics." << endl;
    }
